...
class Cart extends Component {
    ...
    handleClickSubmit() {
        ...
        Swal.fire({
            title: 'Valor Total: ' + this.getTotal(this.state.cart) + ' €',
            input: "text",
            inputAttributes: {
                inputmode: isMobile() ? "decimal" : "text",
            },
            // inputValue: this.getTotal(this.state.cart),
            inputPlaceholder: this.getTotal(this.state.cart),
            cancelButtonText: this.state.translations['cancel_pay'],
            showCancelButton: true,
            confirmButtonText: this.state.translations["confirm_pay"],
            showLoaderOnConfirm: true,
            preConfirm: (amount) => {
                //if amount as , replace with .
                amount = amount.replace(",", ".");
                // Validate and format the input value to allow only up to 2 decimal places
                const regex = /^\d+(\.\d{0,2})?$/;
                if (!regex.test(amount)) {
                    Swal.showValidationMessage('Por favor, insira um valor válido');
                    return false;
                }
                //amount as always 2 decimal places
                amount = parseFloat(amount).toFixed(2);

                //if amount is less than the total but if the amount is greater than the total continue
                // if (amount < this.getTotal(this.state.cart)) {
                //     let diference = this.getTotal(this.state.cart) - amount;
                //     Swal.showValidationMessage('O valor inserido é inferior ao total, faltam:  <span style="font-weight: bold;"> ' + (diference.toFixed(2)) + '</span> €');
                //     return false;
                // }
                ...
                try {
                    ...
                    this.amount = amount;
                    let change = this.amount - this.getTotal(this.state.cart);
                    //change only 2 decimal places

                    Swal.fire({
                        title: "Troco é de " + change.toFixed(2) + " €",
                            ...
                            >
                                ...
                                {customers.map((cus) => (
                                    <option
                                        key={cus.id}
                                        value={cus.id}
                                    >{`${cus.first_name} `}  <img src={`https://omniserver.pro/storage/${cus.avatar}`} width="20" height="20" /></option>
                                ))}
                            </select>
                        </div>
                    </div>
